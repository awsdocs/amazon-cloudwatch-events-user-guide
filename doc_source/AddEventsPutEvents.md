...
The `PutEvents` action sends multiple events to CloudWatch Events in a single request\. Each `PutEvents` request can support a limited number of entries\. For more information, see [CloudWatch Events Limits](cloudwatch_limits_cwe.md)\. The `PutEvents` operation attempts to process all entries in the natural order of the request\. Each event has a unique id that is assigned by CloudWatch Events after you call `PutEvents`\.
